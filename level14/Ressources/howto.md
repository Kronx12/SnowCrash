...
**Pour ce level14 il faut simplement exploit le fichier getflag lui meme, pour se faire je le telecharge via la commande suivante:**

`scp -P 4242 level14@192.168.253.129:/bin/getflag .`

**Et je l'envoie dans ghidra pour en savoir plus sur le code present a l'interieur**
...
**Dans ce code on retrouve l'exacte meme ligne que celle de l'exercise precedant, je me dirige donc vers la meme fonction "ft_des" que j'avais reconstruit dans le main.c. Je trouve egalement une ligne a la suite avec un code dedans qui passe encore par la fonction ft_des je decide donc de le faire passer dans mon main, et comme par magie le code fonctionne :)**
